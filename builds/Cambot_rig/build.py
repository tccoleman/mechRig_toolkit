...
from mechRig_toolkit.builds.Cambot_rig import legs
from mechRig_toolkit.builds.Cambot_rig import deform
...
def run():
    ...
    util.lock_unlock_channels(
        lock=True, attrs=["tx", "ty", "tz", "rx", "ry", "rz", "sx", "sy", "sz"]
    )
    ...
    # Head Rig Build
    # =========================
    connect_control_to_joint(
        "cn_head_ctl", "cn_head_jnt", connections=["rotateX"], connect_offset=True
    )
    ...
    # =========================
    connect_control_to_joint(
        "cn_neck_ctl", "cn_neck_jnt", connections=["rotateY"], connect_offset=True
    )
    ...
    # Antenna Rig Build
    # =========================
    ant_up = antenna_setup(
        "cn_antSide", "cn_antSide_ctl", "lf_antenna_jnt", aim=[0, 1, 0], up=[0, 0, 1]
    )
    ...
    for ns in namespaces:
        if namespace_string in ns:
            try:
                ...
                log.exception(
                    "Could not remove namespace, it might not be empty: {}".format(e)
                )


def connect_control_to_joint(
    control, joint, connections=["rotateX"], connect_offset=True
):
    ...
    if cmds.objExists(control) and cmds.objExists(joint):
        for attr in connections:
            ...
            pma = cmds.createNode(
                "plusMinusAverage", name="{}{}_pma".format(control, attr)
            )
            ...
            if connect_offset:
                ...
                if offset:
                    cmds.connectAttr(
                        "{}.{}".format(offset[0], attr), "{}.input1D[1]".format(pma)
                    )

            cmds.connectAttr(
                "{}.output1D".format(pma), "{}.{}".format(joint, attr), force=True
            )
...
def antenna_setup(base_name, control, joint, aim=[0, 1, 0], up=[0, 0, 1]):
    ...
    cmds.aimConstraint(
        control, joint, wut="object", wuo=ant_up, aimVector=aim, upVector=up
    )
...
def lock_channels():
    ...
    cmds.select(BODY)
    util.lock_unlock_channels(
        lock=True, attrs=["tx", "ry", "rz", "sx", "sy", "sz", "v"]
    )
    ...
    util.lock_unlock_channels(
        lock=True, attrs=["tx", "ty", "tz", "ry", "rz", "sx", "sy", "sz", "v"]
    )

    YROT_ONLY = cmds.ls(
        "cn_neck_off", "cn_neck_ctl", "*_shoulder*_off", "*_shoulder*_ctl"
    )
    cmds.select(clear=True)
    cmds.select(YROT_ONLY)
    util.lock_unlock_channels(
        lock=True, attrs=["tx", "ty", "tz", "rx", "rz", "sx", "sy", "sz", "v"]
    )
    ...
    cmds.select(ANTENNA)
    util.lock_unlock_channels(
        lock=True, attrs=["ty", "rx", "ry", "rz", "sx", "sy", "sz", "v"]
    )
