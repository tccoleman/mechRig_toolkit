...
class load(shelf_base._shelf):
    def build(self):
        ...
        self.addMenuItemDivider(
            general_tools_menu, divider=True, dividerLabel="DISPLAY..."
        )

        self.addMenuItem(
            general_tools_menu,
            "Toggle anti-alias viewport display",
            command="from mechRig_toolkit.utils import general; "
        ...
        self.addMenuItemDivider(
            snap_tools_menu, divider=True, dividerLabel="CREATE LOCATORS..."
        )

        self.addMenuItem(
            snap_tools_menu,
            "Locator at selected position",
        ...
        self.addMenuItem(
            snap_tools_menu,
            "Locator at selected position/rotation",
            command="from mechRig_toolkit.utils import locator; "
        ...
        self.addMenuItemDivider(
            snap_tools_menu, divider=True, dividerLabel="MATCHING TRANSFORMS..."
        )
        ...
        self.addMenuItemDivider(
            joint_tools_menu, divider=True, dividerLabel="CREATE..."
        )
        ...
        self.addMenuItemDivider(
            joint_tools_menu, divider=True, dividerLabel="UTILITIES..."
        )

        self.addMenuItem(
            joint_tools_menu,
            "Toggle LRA",
            command="from maya import cmds; cmds.toggle(localAxis=True)",
        )
        ...
        self.addMenuItemDivider(
            skin_tools_menu, divider=True, dividerLabel="UTILITIES..."
        )
        ...
        self.addMenuItemDivider(
            skin_tools_menu, divider=True, dividerLabel="IMPORT/EXPORT..."
        )
        ...
        cmds.menuItem(
            ...
            command=lambda *args: cmds.duplicate(parentOnly=True),
        )
        cmds.menuItem(
            p=week6_tools_menu,
            l="Snap first object(s) to last",
            command=lambda *args: week6.match_selection(),
        )
        cmds.menuItem(
            p=week6_tools_menu,
            l="Create Pole Vector (select PV control then IK handle)",
            command=lambda *args: week6.create_pole_vector_from_selection(),
        )
        cmds.menuItem(
            p=week6_tools_menu,
            l="Create Category Switch (select rig top node)",
            command=lambda *args: week6.create_category_ui(),
        )
        cmds.menuItem(
            p=week6_tools_menu,
            l="Add offset and group transforms above selected control",
            command=lambda *args: week6.add_transforms_selected(),
        )
