...
def create_pole_vector_from_selection():
    ...
    if len(selection) == 2:
        ...
        log.info(
            "Created pole vector constraint from {} to {}".format(pv_ctl, ik_handle)
        )
        return True
...
def create_pole_vector(pv_ctl, ik_handle):
    ...
    cmds.delete(
        cmds.aimConstraint(
            mid_joint[0], pv_ctl, aim=[1, 0, 0], u=[0, 0, 1], wut="object", wuo=end_eff
        )
    )
    ...
    if cmds.objExists(top_node):
        if not cmds.objExists(category_name):
            ...
            cond_node = cmds.createNode(
                "condition", name="{}_{}_cond".format(top_node, category_name)
            )
            cmds.connectAttr(
                "{}.{}".format(top_node, category_name),
...
def add_transforms_selected():
    ...
    if selection:
        ...
        log.error(
            "No transform(s) selected, select at least one transform and try again..."
        )
        return
    ...
    for tfm in transform_list:
        if cmds.nodeType(tfm) == "transform":
            ...
            for i in range(0, len(add_transforms)):
                add_tfm = cmds.duplicate(
                    tfm, po=True, name=tfm.replace(search, add_transforms[i])
                )[0]
            ...
            log.info(
                "Added transforms {} to {}".format(
                    ", ".join(map(str, created_tfms)), tfm
                )
            )
